...
it('lets a manager add a website URL and dispatches a crawl', function () {
    ...
    $this->actingAs($user)
        ->post(route('bots.sources.store', ['current_team' => $team->slug, 'bot' => $bot->id]), [
    ...
    Queue::assertPushed(CrawlSiteJob::class, fn (CrawlSiteJob $job) => $job->botId === $bot->id
...
it('rejects URLs that target private or internal addresses', function () {
    ...
    foreach (['http://127.0.0.1/admin', 'http://169.254.169.254/latest/meta-data', 'http://10.0.0.5', 'ftp://example.com/file'] as $url) {
        $this->actingAs($user)
            ->post(route('bots.sources.store', ['current_team' => $team->slug, 'bot' => $bot->id]), ['url' => $url])
            ->assertInvalid(['url']);
    }
...
it('lets a manager delete a document but not one from another bot', function () {
    [$user, $team] = ingestionMember(TeamRole::Owner);
    $bot = Bot::factory()->for($team)->create();
    ...
    $this->actingAs($user)
        ->delete(route('bots.documents.destroy', ['current_team' => $team->slug, 'bot' => $bot->id, 'document' => $otherDocument->id]))
        ->assertNotFound();

    $this->actingAs($user)
        ->delete(route('bots.documents.destroy', ['current_team' => $team->slug, 'bot' => $bot->id, 'document' => $document->id]))
        ->assertRedirect();
...
it('re-crawls a bot by resetting its web documents to pending', function () {
    Queue::fake();
    ...
    $this->actingAs($user)
        ->post(route('bots.recrawl', ['current_team' => $team->slug, 'bot' => $bot->id]))
        ->assertRedirect();

    expect($bot->documents()->where('status', DocumentStatus::Pending)->count())->toBe(1);
...
it('crawls a sitemap into done documents with chunk text, always including the seed', function () {
    ...
    (new CrawlSiteJob($bot->id, PUBLIC_HOST.'/'))->handle();

    // page-1, page-2, plus the seed itself.
    expect($bot->documents()->count())->toBe(3)
        ->and($bot->documents()->where('source_url', PUBLIC_HOST.'/')->exists())->toBeTrue()
        ->and($bot->documents()->where('status', DocumentStatus::Processing)->count())->toBe(3);
...
it('does not duplicate documents when a crawl re-runs', function () {
    ...
    expect($bot->documents()->count())->toBe(2);
});
...
it('fails documents gracefully when the site has no sitemap', function () {
    ...
    $bot = Bot::factory()->create();
    $seed = Document::factory()->for($bot)->create([
        'type' => 'web',
