 ...
 */
function embeddedAgent(int $chunkCount = 3): array
{
    ...
    Chunk::factory()->for($agent)->for($document)->count($chunkCount)->create([
...
it('returns top-N chunks ordered by rerank score', function () {
    ...
    [$agent] = embeddedAgent(chunkCount: 3);

    $result = retriever()->retrieve($agent, 'How do refunds work?', topN: 6);
...
it('only returns chunks belonging to the queried agent', function () {
    Embeddings::fake();
    Reranking::fake();

    [$agentA] = embeddedAgent(chunkCount: 3);
    [$agentB] = embeddedAgent(chunkCount: 3);

    $result = retriever()->retrieve($agentA, 'anything');

    $agentAChunkIds = $agentA->chunks()->pluck('id')->all();

    foreach ($result->hits as $hit) {
        expect($agentAChunkIds)->toContain($hit->chunkId);
    }
})->repeat(3);
...
it('falls back to vector order when no rerank provider key is configured', function () {
    ...
    [$agent] = embeddedAgent(chunkCount: 4);

    $result = retriever()->retrieve($agent, 'a question', topN: 2);
...
it('returns an empty result with zero top score when nothing is embedded', function () {
    ...
    $result = retriever()->retrieve($agent, 'a question');
